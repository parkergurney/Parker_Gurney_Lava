...
const page = () => {
	...
	return (
		<div className='flex'>
			...
			{isSidebarOpen && (
				<div className={`fixed inset-0 z-10 ${isSidebarOpen ? 'bg-black/10' : 'bg-transparent'} transition-all duration-500`}
					onClick={() => setIsSidebarOpen(false)} />
			)}
			{isFormOpen && (
				<div className="fixed inset-0 z-10 bg-transparent" onClick={() => setIsFormOpen(false)} />
			)}
			<div className='flex-1 w-full flex justify-center'>
				<div className='pl-[48px] flex flex-col w-full max-w-screen-lg mx-4 pt-[36px] space-y-[12px]'>
					<div className={`w-full h-[80px] sm:h-[41px] flex flex-col sm:flex-row items-center sm:justify-between ${isFormOpen ? 'blur-sm transition-all' : 'transition-all'}`}>
